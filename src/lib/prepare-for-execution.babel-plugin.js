...
// This plugin marks every file that imports the `css` function as a static file.
// It also adds a UUID to every `css` invocation as the last argument.
const importDeclarationVisitor = types => (path, { opts, file }) => {
	// Make sure that the import is from "~stitches"
	if (!checkImport(path)) return
	...
	*/
	...
	if (isStatic) opts.markFileAsStatic()
...
function checkImport(path) {
	...
	if (importSource !== "~stitches") return false
	return true
}
